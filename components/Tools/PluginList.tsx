...
const ToolCard = ({tool, disabled}:{tool:Tool, disabled:boolean})=>{
    return(
        ...
        <Card 
        className={`w-full h-full align-start items-start text-sm ${
            disabled ? 'opacity-50' : ''
        }`}
        >
                <Flex height={'100%'}>
                    <Flex direction="column" width="100%">
                        <Flex direction="column" gap="1" className='mb-4'>
                            <Text weight="bold" className="text-base">{tool.display || tool.function.name}</Text>
                            <Text size="1" className="italic">{tool.plugin || 'Common'}</Text>
                        </Flex>
