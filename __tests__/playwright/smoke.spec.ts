...
test('test', async ({ page }) => {
  ...
  await page.locator('div').filter({ hasText: /^New Chat$/ }).click();
  await page.locator('div').filter({ hasText: /^Untitled$/ }).nth(1).click();
  ...
  await page.locator('div').filter({ hasText: /^AutoHello!$/ }).getByRole('button').first().click();
